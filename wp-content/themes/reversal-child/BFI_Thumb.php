	...
	class BFI_Image_Editor_GD_1_3 extends WP_Image_Editor_GD {
	    ...
	    public function brightness( $value ) {
	        ...
	        if ( function_exists( 'imagefilter' ) &&
	            ...
	            imagealphablending( $this->image, false );
	            if ( imagefilter( $this->image, IMG_FILTER_BRIGHTNESS, $value ) ) {
	                imagesavealpha( $this->image, true );
	                return true;
	            }
	        }
	        return new WP_Error( 'image_brightness_error', __( 'Image brightness change failed.', 'default' ), $this->file );
	    }

	    /** Alters the image's Contrast
	     *
	     * @supports 3.5.1
	     * @access public
	     *
	    ...
	    public function contrast( $value ) {
	        ...
	        if ( function_exists( 'imagefilter' ) &&
	            ...
	            imagealphablending( $this->image, false );
	            if ( imagefilter( $this->image, IMG_FILTER_CONTRAST, $value ) ) {
	                imagesavealpha( $this->image, true );
	                return true;
	            }
	        }
	        return new WP_Error( 'image_contrast_error', __( 'Image contrast change failed.', 'default' ), $this->file );
	    }
	...
	class BFI_Thumb_1_3 {
	    ...
	    public static function thumb( $url, $params = array(), $single = true ) {
	        ...
	        $suffix = (string) filemtime( $img_path ) .
	            ...
	            ( isset( $brightness ) ? str_pad( (string) ($brightness+255), 3, '0', STR_PAD_LEFT ) : '255' ) .
	            ( isset( $contrast ) ? str_pad( (string) ($contrast+100), 3, '0', STR_PAD_LEFT ) : '100' ) .
	        ...
	        if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
	            ...
	            if ( isset( $brightness) ) {
	                if ( is_wp_error( $editor->brightness( $brightness ) ) ) {
	                    return false;
	                }
	            }
	            
	            if ( isset( $contrast ) ) {
	                if ( is_wp_error( $editor->contrast( $contrast ) ) ) {
	                    return false;
	                }
	            }
